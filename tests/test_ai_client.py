...
def test_summarize_reflection_uses_openrouter_response(monkeypatch) -> None:
    monkeypatch.setattr(ai_client, 'load_settings', lambda: real_load_settings(_env()))
    monkeypatch.setattr(
        ai_client.request,
        'urlopen',
    ...
    text = ai_client.summarize_reflection({'scenario': 'checkin', 'cards': ['Тихий маяк']})
...
def test_suggest_small_step_falls_back_on_timeout(monkeypatch) -> None:
    ...
    text = ai_client.suggest_small_step({'scenario': 'checkin', 'focus': 'Тихий маяк'})
...
def test_summarize_reflection_falls_back_on_bad_response(monkeypatch) -> None:
    ...
    text = ai_client.summarize_reflection({'scenario': 'situation', 'cards': ['Сад после дождя']})

    assert 'Сад после дождя' in text


def test_ai_client_returns_fallback_when_disabled(monkeypatch) -> None:
    monkeypatch.setattr(ai_client, 'load_settings', lambda: real_load_settings(_env(AI_ENABLED='0')))

    with patch.object(ai_client.request, 'urlopen') as mocked:
        text = ai_client.suggest_small_step({'scenario': 'checkin', 'prompts': ['Что важно?']})
