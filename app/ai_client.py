...
def summarize_reflection(context: dict[str, Any], settings: Settings | None = None) -> str:
    return _generate(
        task="summary",
        context=context,
...
def suggest_small_step(context: dict[str, Any], settings: Settings | None = None) -> str:
    return _generate(
        task="small_step",
        context=context,
...
def _generate(
    ...
    if not resolved_settings.ai_enabled:
        return fallback
    if resolved_settings.ai_provider != "openrouter":
        logger.warning("Unsupported AI provider '%s'; using fallback", resolved_settings.ai_provider)
        return fallback
    ...
    try:
        response_text = _request_openrouter(
            context=context,
